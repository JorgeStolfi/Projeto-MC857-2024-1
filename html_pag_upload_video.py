...
def gera(ses, atrs, erros):
  ...
  POST serão 'arq' (nome do arquivo), 'titulo' (o título), e
  'conteudo' (o conteúdo do arquivo).
  
  O parâmetro {atrs} deve ser um dict que opcionalmente especifica
  valores inicial para os campos 'arq' e/ou 'titulo'.  O parâmetro
  {erros} deve ser {None} ou uma lista de mensagens (strings) a mostrar
  na página.

  Os parâmetros {args} e {erros} são usados quando tentativa de fazer
