...
def processa(ses, cmd_args):
    ...
    O parâmetro {ses} não pode ser {None} e
    deve ser um objeto de tipo {obj_sessao.Class}. A sessão deve estar aberta.
    Se o dono da sessão {ses} for um administrador e {cmd_args} tiver um campo 'id_usr' não nulo,
    este campo define o usuário {usr} a alterar. Caso contrário, o usuário {usr}
    será o próprio dono da sessão {ses}.
